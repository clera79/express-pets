...
exports.submitContact = async function(req, res, next) {
    ...
    // Check user enter "puppy" as secret word
    ...
    // Check whether Mongo recognizes pet ID
    if (!ObjectId.isValid(req.body.petId)) {
        console.log("Inavlid ID detected");
        return res.json({message: "Sorry!"});
    }

    // Check whether record exists within the pets collection
    ...
    // Sanitize user submitted data in the contact form
    const ourObject = {
    ...
    // Copied from Mailtrap website (Code Sample --> Node.js)
    ...
    try {
        // Email to user that has submitted the contact form
        const promise1 = transport.sendMail({
            ...
            <p>We appreciate your interest in ${doesPetExist.name} and one of our staff memmbers will reach out to you shortly. Below is a copy of the message you sent us for your personal records:</p>
            <em>${ourObject.comment}</em>
            `
        });

        // Email to admin / owner of the site
        const promise2 = transport.sendMail({
            ...
            Email address: ${ourObject.email}<br>
